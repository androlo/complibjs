...
import {materializeConstSparseAddSub, materializeTensorDenseConst} from "../src/materialize";
...
describe('materializeTensorDenseConst', () => {
    const cf = getCompFunc();
    // Build a 1D dense from sparse for convenience (same domain as typical sparse):
    ...
    it('shape & length for dimC=1', () => {
        ...
        const out = materializeTensorDenseConst(d1, c1)! as CFUnitFuncDense<CFDim>;
        expect(out.storage).toBe(CFStorageTag.Dense);
        expect(out.dim).toBe((d1.dim + 1) as CFDim);
        expect(out.NU).toBe(NU);
        expect(out.NS).toBe(NS);

        const pows = generatePowerArray(((d1.dim + 1) + 1) as CFUint32, NU)!;
    ...
    it('pointwise correctness for dimC=1', () => {
        ...
        const out = materializeTensorDenseConst(d1, c1)! as CFUnitFuncDense<CFUint32Two>;
        ...
        for (let s = 0 as CFSeriesIndex; s < maxS; s = (s + 1) as CFSeriesIndex) {
            for (let uL = 0 as CFUnit; uL < maxU; uL = (uL + 1) as CFUnit) {
                for (let uR = 0 as CFUnit; uR < maxU; uR = (uR + 1) as CFUnit) {
                    ...
                    const expected = ALGEBRA_IVAL.mul(d1.getUnsafe(uL, s), c1.getUnsafe(uR, s));
    ...
    it('dimC=2 quick spot-check', () => {
        ...
        const out = materializeTensorDenseConst(d1, c2)! as CFUnitFuncDense<CFUint32Three>;
            ...
            for (let uL = 0 as CFUnit; uL < maxU; uL = (uL + 1) as CFUnit) {
                for (let u1 = 0 as CFUnit; u1 < maxU; u1 = (u1 + 1) as CFUnit) {
                    for (let u2 = 0 as CFUnit; u2 < maxU; u2 = (u2 + 1) as CFUnit) {
                        ...
                        const expected = ALGEBRA_IVAL.mul(d1.getUnsafe(uL, s), c2.getUnsafe(u1, u2, s));
                        expect(ALGEBRA_IVAL.eq(got, expected)).toBe(true);
                    }
                }
            }
